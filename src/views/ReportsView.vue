        ...
        <div class="min-h-screen p-6">
            ...
            <Card class="mt-5 shadow-xl border-[.5px] p-1">
                <template #title>
                    <div class="flex flex-col gap-4 text-xs font-normal xl:justify-between xl:gap-2 xl:flex-row">
                        <div class="inline-flex flex-col justify-between gap-2 sm:flex-row ">
                            ...
                            <Dropdown v-model="selectedUser" :options="users" option-label="name"
                                placeholder="Select User" class="w-full h-10 p-0 m-0 !rounded-full" />
                        ...
                        <div class="flex flex-col justify-between border rounded-md sm:flex-row">
                            <div class="inline-flex items-baseline w-full p-1">
                                ...
                                <Calendar class="w-full h-10" date-format="d M. yy" :manual-input="false"
                                    v-model="fromDate" show-button-bar="" show-icon iconDisplay="input" />
                            </div>
                            <div class="inline-flex items-baseline w-full p-1">
                                <span class="w-8 m-1 font-semibold">To</span>
                                <Calendar class="w-full h-10" date-format="d M. yy" :manual-input="false"
                                    v-model="toDate" show-button-bar="" show-icon iconDisplay="input" />
                            </div>
                    ...
                    <div class="flex items-start justify-between mt-4 text-base font-normal">
                        <h2 class="text-sm">Displaying Range from <br class="inline md:hidden"> <span
                                class="font-semibold">{{ formatDate(fromDate) }}</span> to <span
                                class="font-semibold">{{
                ...
                <template #content>
                    ...
                    <DataTable class="text-sm" :value="reportEntries" paginator :rows="10"
                        ...
                        <Column field="status" header="Status">
                            <template #body="slotProps">
                                ...
                                <Tag class="w-full" v-else-if="slotProps.index % 2 == 0" severity="danger"
                                    value="Failed">
                                </Tag>
                                <Tag class="w-full bg-primary-100 !text-primary" v-else severity="warning"
                                    value="Pending">
                                </Tag>
                            </template>
                        </Column>
